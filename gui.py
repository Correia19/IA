...
from warehouse.actions import ActionLeft, ActionRight, ActionUp, ActionDown
...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        for pair in self.agent.pairs:
            problem = WarehouseProblemSearch(self.gui.initial_state, pair.cell2)
            problem.initial_state.line_forklift = pair.cell1.line
            problem.initial_state.column_forklift = pair.cell1.column
            solution = self.agent.solve_problem(problem)
            pair.value = solution.cost

            pair.actions = solution.actions

            current_line = pair.cell1.line
            current_column = pair.cell1.column
            if(problem.initial_state.matrix[current_line][current_column] == constants.FORKLIFT):
                pair.path.append((current_line, current_column))
            for action in pair.actions:
                # o que isto diz é se dentro disto for true entao faz
                # exemplo
                # a = 12
                # isinstance(a, int) vai ser verdade
                # dentro da action vai ter um valor warehouse.actions.ActionLeft entao se o valor lá dentro for ActionLeft faz
                if isinstance(action, ActionLeft):
                    current_column -= 1
                elif isinstance(action, ActionRight):
                    current_column += 1
                elif isinstance(action, ActionUp):
                    current_line -= 1
...
class SolutionRunner(threading.Thread):
    ...
    def run(self):
        self.thread_running = True
        forklift_path, steps, genome = self.best_in_run.obtain_all_path()
        ...
        for gene in genome:
            if gene > len(genome):
                produtosPorForklift.append(produtos)
                produtos = []
            else:
                produtos.append(gene)
        produtosPorForklift.append(produtos)
            ...
            for j in range(len(forklift_path)):
                ...
                if step < len(forklift_path[j]) - 1:
                    if old_cell[j] not in new_cells:
                        self.state.matrix[old_cell[j][0]][old_cell[j][1]] = constants.EMPTY
                    new_cell = forklift_path[j][step + 1]
                    new_cells.append(new_cell)
                    self.state.matrix[new_cell[0]][new_cell[1]] = constants.FORKLIFT
                    old_cell[j] = new_cell
                else:
                    self.state.matrix[old_cell[j][0]][old_cell[j][1]] = constants.FORKLIFT

                current_line = new_cell[0]
                current_column = new_cell[1]
                cellProdutoEsquerda = ((current_line, current_column-1))
                cellProdutoDireita = ((current_line, current_column+1))
                # Catch products and mark them as black
                if ((cellProdutoEsquerda[1] < self.state.columns) and(self.state.matrix[cellProdutoEsquerda[0]][cellProdutoEsquerda[1]] == constants.PRODUCT)):
                    if(new_cell in forklift_path[j]):
                        for i in range(len(self.best_in_run.problem.products)):
                            if((cellProdutoEsquerda[0] == self.best_in_run.problem.products[i].line and cellProdutoEsquerda[1] == self.best_in_run.problem.products[i].column)):
                                produtoIndex = i
                                break
                        if (len(produtosPorForklift[j]) > 0):
                            if(produtoIndex == produtosPorForklift[j][0]):
                                self.state.matrix[cellProdutoEsquerda[0]][cellProdutoEsquerda[1]] = constants.PRODUCT_CATCH
                                produtosPorForklift[j].pop(0)
                if ((cellProdutoDireita[1] < self.state.columns) and(self.state.matrix[cellProdutoDireita[0]][cellProdutoDireita[1]] == constants.PRODUCT)):
                    if (new_cell in forklift_path[j]):
                        for i in range(len(self.best_in_run.problem.products)):
                            if((cellProdutoDireita[0] == self.best_in_run.problem.products[i].line and cellProdutoDireita[1] == self.best_in_run.problem.products[i].column)):
                                produtoIndex = i
                                break
                        if (len(produtosPorForklift[j]) > 0):
                            if(produtoIndex == produtosPorForklift[j][0]):
                                 self.state.matrix[cellProdutoDireita[0]][cellProdutoDireita[1]] = constants.PRODUCT_CATCH
                                 produtosPorForklift[j].pop(0)
